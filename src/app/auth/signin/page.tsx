...
const SignIn: React.FC = () => {
  ...
  useEffect(() => {
    console.log(signinstate, "state");
  }, [signinstate]);

  useEffect(() => {
    if (signinstate && signinstate.redirectUrl) {
      if (signinstate.message) {
       
        showToast(signinstate.message, 'success');
      }
      redirect(signinstate.redirectUrl);
    }

    if (signinstate && signinstate.message) {
      console.log(signinstate.message, "msg");
      //  window.alert(state.message)
      //toast.success(state.message);
      if (signinstate.message == "Invalid email or password") {
        showToast(signinstate.message, 'error');
          ...
          <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
            ...
            <form action={signinaction}>
              ...
              {signinstate?.errors?.email && (
                <p className="text-red">{signinstate.errors.email}</p>
              )}
